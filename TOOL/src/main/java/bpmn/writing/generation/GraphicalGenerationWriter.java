package bpmn.writing.generation;
...
import bpmn.writing.generation.service.RuntimeValidationService;
import bpmn.writing.generation.transformer.PifContentTransformer;
...
public class GraphicalGenerationWriter
{
    ...
    private static final String REFACTORED_BPMN_FILE = "refactored_process.bpmn";
    ...
    {
        final File finalFile = new File((Paths.get(((File) this.commandLineParser.get(CommandLineOption.WORKING_DIRECTORY)).getPath(), this.dumpValue.isEmpty() ? REFACTORED_BPMN_FILE : ("refactored_process_" + this.dumpValue + ".bpmn")).toString()));
    ...
    private void correctFinalBpmnFile()
    {
        ...
        final File generatedFile = new File((Paths.get(((File) this.commandLineParser.get(CommandLineOption.WORKING_DIRECTORY)).getPath(), this.dumpValue.isEmpty() ? REFACTORED_BPMN_FILE : ("refactored_process_" + this.dumpValue + ".bpmn")).toString()));
